...
const toDevice = (d) => ({
  ...
  publicHost: d.public_host || null, publicPort: d.public_port || null, lastLogCount: d.last_log_count || 0,
})
...
module.exports = {
  ...
  punches: {
    ...
    // { deviceId: { n, lastDetectedAt } } — quarantined-punch counts per device
    // (device_corrupted_counts view). Powers the Devices page's broken-clock
    // warning chip; see corrupted_punches in schema.sql for why these exist.
    corruptedCountsByDevice: async () => {
      const { data, error } = await sb.from('device_corrupted_counts').select('device_id,n,last_detected_at')
      if (error) throw new Error(error.message)
      return Object.fromEntries(data.map(r => [r.device_id, { n: r.n, lastDetectedAt: r.last_detected_at }]))
    },
  ...
  employees: {
    ...
    set: async (pin, data) => {
      ...
      const curE = cur ? toEmployee(cur) : { name: '', group: null, timeIn: null, timeOut: null, offDays: [0, 6], aliases: [] }
      ...
      const row = {
        ...
        aliases: data && 'aliases' in data && Array.isArray(data.aliases) ? data.aliases : curE.aliases,
      }
    ...
    // Link another device PIN to this person (same human, non-global PIN).
    // Punches under the alias roll up to primaryPin at read time — history is
    // NOT re-pointed. If aliasPin was its own staff row, it's folded in (deleted).
    addAlias: async (primaryPin, aliasPin) => {
      primaryPin = String(primaryPin); aliasPin = String(aliasPin || '').trim()
      if (!aliasPin) throw new Error('Alias PIN is required.')
      if (aliasPin === primaryPin) throw new Error('A PIN cannot be an alias of itself.')
      const { data: prim } = await sb.from('employees').select('*').eq('pin', primaryPin).maybeSingle()
      if (!prim) throw new Error('Primary staff member not found.')
      const { data: owners } = await sb.from('employees').select('pin').contains('aliases', [aliasPin])
      if (owners && owners.some(o => o.pin !== primaryPin)) throw new Error(`PIN ${aliasPin} is already linked to another staff member.`)
      const aliases = Array.isArray(prim.aliases) ? prim.aliases.slice() : []
      if (!aliases.includes(aliasPin)) aliases.push(aliasPin)
      const { error } = await sb.from('employees').update({ aliases }).eq('pin', primaryPin)
      if (error) throw new Error(error.message)
      await sb.from('employees').delete().eq('pin', aliasPin)   // fold in any separate row
    },
    removeAlias: async (primaryPin, aliasPin) => {
      primaryPin = String(primaryPin); aliasPin = String(aliasPin)
      const { data: prim } = await sb.from('employees').select('aliases').eq('pin', primaryPin).maybeSingle()
      if (!prim) return
      const aliases = (Array.isArray(prim.aliases) ? prim.aliases : []).filter(a => a !== aliasPin)
      const { error } = await sb.from('employees').update({ aliases }).eq('pin', primaryPin)
      if (error) throw new Error(error.message)
    },
  },
