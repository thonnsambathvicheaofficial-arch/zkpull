...
module.exports = {
  ...
  punches: {
    ...
    // { deviceId: count } — used for the Devices page's per-device punch totals.
    countsByDevice: async () => {
      const rows = await selectAll('punches', q => q.select('device_id'))
      const counts = {}
      for (const r of rows) if (r.device_id) counts[r.device_id] = (counts[r.device_id] || 0) + 1
      return counts
    },
