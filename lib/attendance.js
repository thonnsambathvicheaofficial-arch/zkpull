...
function dailyRows(punches, employees, settings, overrides, from, to) {
  ...
  for (const [pin, times] of timesByPin) {
    ...
    for (const s of buildSessions(times, rule)) {
      ...
      // ── MIDNIGHT ROLLOVER ────────────────────────────────────────────────
      // When the OUT punch falls on a different calendar day than the IN, we
      // split the session in two instead of flagging "+N day":
      //   • The IN day gets a present/late shift with no OUT (missing OUT).
      //   • The OUT punch is rolled over to its own date as an outOnly entry
      //     (the worker was clearly there — they scanned out — but no IN was
      //     recorded on that day because they started the previous day).
      // This removes the +1 day badge and makes the next-day row show the OUT
      // time rather than showing as fully absent.
      if (s.in && outDate && outDate !== date) {
