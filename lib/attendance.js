...
//
// rule is optional but when provided it prevents a common mis-classification:
// a day worker's 5 PM OUT and next morning's 7 AM IN are ~14h apart and fall
// within MAX_SHIFT_HOURS — without the rule they're merged into a fake night
// shift. When the next scan crosses a midnight boundary AND its time-of-day is
// close to the staff's scheduled start AND the gap from the PREVIOUS scan is
// large enough to indicate a rest period, we break the session there instead.
function buildSessions(scanTimes, rule = null) {
  ...
  // If the worker has a configured (or default) start time we use it to detect
  // a next-day morning arrival. 2-hour window handles early/late arrivals well.
  ...
  while (i < scans.length) {
    ...
    while (j + 1 < scans.length) {
      ...
      // Midnight-crossing split: if the next scan is on a different calendar
      // day, the gap from the LAST kept scan is > 6 h (a genuine rest period),
      // and the next scan's time-of-day is within 2 h of the scheduled start,
      // treat it as the opening punch of a new shift, not an extension of this one.
      if (startMin !== null &&
