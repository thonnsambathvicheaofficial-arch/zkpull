...
// "YYYY-MM-DD" -> the following calendar day, same string format.
const nextDate = (dateStr) => { const d = new Date(dateStr + 'T00:00:00'); d.setDate(d.getDate() + 1); return fmtDate(d) }
      ...
      if (ov) {
        ...
        if (!zeroHours) {
          if (ov.timeIn && ov.timeOut) {
            ...
            // A manual correction can describe an overnight shift (e.g. IN
            // 16:35 -> OUT 00:00). If the out time is numerically earlier
            // than the in time, treat it as the FOLLOWING calendar day —
            // same convention resolveShift() uses for a real overnight shift
            // — so a night-shift override doesn't compute a negative/bogus
            // duration from both times being pinned to the same date.
            const crossesMidnight = hhmmToMin(ov.timeOut) < hhmmToMin(ov.timeIn)
            const outDate = crossesMidnight ? nextDate(date) : date
            const mins = diffMinutes(`${date} ${ov.timeIn}:00`, `${outDate} ${ov.timeOut}:00`)
            hours = Math.max(0, Math.round(mins / 6) / 10)
            allPunches = [`${date} ${ov.timeIn}:00`, `${outDate} ${ov.timeOut}:00`]
            punchCount = allPunches.length   // pre-existing gap: this was never set for a manual time_in/time_out override, always showing "0 punches"
            outNextDay = crossesMidnight
            outDayOffset = crossesMidnight ? 1 : 0
          } else if (shift) {
